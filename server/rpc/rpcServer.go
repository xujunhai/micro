...
func (s *rpcServer) ServeConn(sock transport.Socket) {
	...
	defer func() {
		...
		// recover any panics
		if r := recover(); r != nil {
			if logger.V(core.ErrorLevel, logger.DefaultLogger) {
...
//TODO direct call newRpcHandler?
...
func (s *rpcServer) Register() error {
	...
	if !registered {
		if logger.V(core.InfoLevel, logger.DefaultLogger) {
...
func (s *rpcServer) Start() error {
	...
	if err = s.opts.RegisterCheck(s.opts.Context); err != nil {
		...
		// if register error should panic service？
		if err = s.Register(); err != nil {
			if logger.V(core.ErrorLevel, logger.DefaultLogger) {
				logger.Errorf("Server %s-%s register error: %s", config.Name, config.Id, err)
			}
			panic(fmt.Sprintf("Server %s-%s register error: %s", config.Name, config.Id, err))
		}
	}
