...
func newFileSystemDynamicConfiguration(url *common.URL) (*FileSystemDynamicConfiguration, error) {
	...
	if _, err := os.Stat(root); err != nil {
		// not exist, use default, /XXX/xx/micro/config
		if rp, err := Home(); err != nil {
			return nil, perrors.WithStack(err)
		} else {
			root = path.Join(rp, "micro", "config")
		}
	}
...
func (fsdc *FileSystemDynamicConfiguration) AddListener(key string, listener config.ConfigurationListener,
	opts ...config.Option) {
...
func (fsdc *FileSystemDynamicConfiguration) RemoveListener(key string, listener config.ConfigurationListener,
	opts ...config.Option) {
	tmpOpts := &config.Options{}
	for _, opt := range opts {
		opt(tmpOpts)
...
func (fsdc *FileSystemDynamicConfiguration) GetProperties(key string, opts ...config.Option) (string, error) {
	tmpOpts := &config.Options{}
	for _, opt := range opts {
		opt(tmpOpts)
	}
...
func (fsdc *FileSystemDynamicConfiguration) GetRule(key string, opts ...config.Option) (string, error) {
	return fsdc.GetProperties(key, opts...)
}

// GetInternalProperty get value by key in Default properties file(dubbo.properties)
func (fsdc *FileSystemDynamicConfiguration) GetInternalProperty(key string, opts ...config.Option) (string,
	error) {
	return fsdc.GetProperties(key, opts...)
}
