...
var (
	// Nodes service node
	Nodes = [...]string{"consumers", "configurators", "routers", "providers"}
	// Role Dubbo service role
	Role = [...]string{"consumer", "", "routers", "provider"}
)
...
// UrlOption accepts url
// UrlOption will define a function of handling URL
type UrlOption func(*URL)

// WithUsername sets username for url
func WithUsername(username string) UrlOption {
	return func(url *URL) {
...
func WithPassword(pwd string) UrlOption {
	return func(url *URL) {
		url.Password = pwd
	}
}
...
func WithMethods(methods []string) UrlOption {
	return func(url *URL) {
		url.Methods = methods
	}
}
...
func WithParams(params url.Values) UrlOption {
	return func(url *URL) {
		url.params = params
	}
}
...
func WithParamsValue(key, val string) UrlOption {
	return func(url *URL) {
...
func WithProtocol(proto string) UrlOption {
	return func(url *URL) {
		url.Protocol = proto
	}
}
...
func WithIp(ip string) UrlOption {
	return func(url *URL) {
		url.Ip = ip
	}
}
...
func WithPort(port string) UrlOption {
	return func(url *URL) {
		url.Port = port
	}
}
...
func WithPath(path string) UrlOption {
	return func(url *URL) {
...
func WithLocation(location string) UrlOption {
	return func(url *URL) {
...
func WithToken(token string) UrlOption {
	return func(url *URL) {
		if len(token) > 0 {
			value := token
...
// NewURLWithUrlOptions will create a new url with options
func NewURLWithUrlOptions(opts ...UrlOption) *URL {
	newUrl := &URL{}
...
func NewURL(urlString string, opts ...UrlOption) (*URL, error) {
...
func MergeUrl(serviceUrl *URL, referenceUrl *URL) *URL {
	...
	methodConfigMergeFcn := mergeNormalParam(mergedUrl, referenceUrl, []string{constant.LoadbalanceKey, constant.ClusterKey, constant.RetriesKey, constant.TimeoutKey})

	// remote timestamp
	if v := serviceUrl.GetParam(constant.TimestampKey, ""); len(v) > 0 {
		mergedUrl.SetParam(constant.RemoteTimestampKey, v)
		mergedUrl.SetParam(constant.TimestampKey, referenceUrl.GetParam(constant.TimestampKey, ""))
	}
